...
export class HomeComponent implements OnInit {
  ...
  getMateriasEstudiante() {
    this.consultEstudianteService.getEstudiante().subscribe({
      next: data => {
        this.listaMateriasEstudiante = data.filter(
          data => Number(data.cedula) === Number(this.cedula)
        );
  ...
  complementData(listMaterias: Materia[]) {
    listMaterias.map(mat => {
      let profes = this.listProfesores.filter(
        prof => prof.id_Materia1 === mat.id || prof.id_Materia2 === mat.id
      );
  ...
  getProfesoresMateria(id: number, index: number) {
    this.resetDetails();
    let profes = this.listProfesores.filter(
      prof => prof.id_Materia1 === id || prof.id_Materia2 === id
    );
  ...
  acceptMaterias(): boolean {
    ...
    let arrayCount = this.listMaterias.filter(
      mat => mat.checked === true
    ).length;
    let count = 0;
    if (arrayCount >= 3) {
      Swal.fire({
  ...
  deleteSelection(item: Estudiante) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.consultEstudianteService
          .deleteMateriaEstudiante(item.id || 0)
          .subscribe({
            next: data => {
            ...
            error: error => {
              if (error.status === 200) {
                ...
                Swal.fire(
                  'Error al momento de eliminar la materia',
                  '',
                  'error'
                );
              }
